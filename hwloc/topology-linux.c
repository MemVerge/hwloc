...
static void dmi_memory_device_size(char *buffer, size_t len, uint16_t code, uint32_t ext_code)
{
  ...
  if (code == 0x7FFF) {
    memory_size = ext_code & 0x7FFFFFFF;
    memory_size <<= 20;
  } else {
  ...
  snprintf(buffer, len, "%lu", memory_size);
}
					...
					struct hwloc_firmware_dmi_mem_device_header *header)
{
  ...
  dmi_memory_device_size(buffer, sizeof(buffer), *(uint16_t *)(&header->size), *(uint32_t *)(&header->extended_size));
